...
export class ReplyService {
  ...
  protected convertDateFromClient(reply: IReply): IReply {
    const copy: IReply = Object.assign({}, reply, {
      dateAndTime: reply.dateAndTime != null && reply.dateAndTime.isValid() ? reply.dateAndTime.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((reply: IReply) => {
        reply.dateAndTime = reply.dateAndTime != null ? moment(reply.dateAndTime) : null;
      });
    }
    return res;
  }

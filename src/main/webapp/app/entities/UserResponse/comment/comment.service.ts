...
export class CommentService {
  ...
  protected convertDateFromClient(comment: IComment): IComment {
    const copy: IComment = Object.assign({}, comment, {
      dateAndTime: comment.dateAndTime != null && comment.dateAndTime.isValid() ? comment.dateAndTime.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((comment: IComment) => {
        comment.dateAndTime = comment.dateAndTime != null ? moment(comment.dateAndTime) : null;
      });
    }
    return res;
  }
